...
def get_tf_translation_once():
    ...
    cmd = ['ros2', 'run', 'tf2_ros', 'tf2_echo', 'proto2/world', 'proto2/base_link']
...
def copy_to_install(src_path):
    ...
    dst_path = os.path.expanduser(
        '~/workspace/riza/ros2_ws/install/carry_and_release/'
        'share/carry_and_release/config/carry_and_release.config.yaml'
    )
...
def main():
    file_path = os.path.expanduser('~/workspace/riza/ros2_ws/src/behavior/carry_and_release/config/carry_and_release.config.yaml')
    ...
    # --- copy to install share path ---
    copy_to_install(file_path)

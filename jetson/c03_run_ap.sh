...
tmux send-keys -t "${SESSION}:0.0" '. ~/workspace/banpaku_scripts/jetson/docker_run_ap.sh' C-m
...
tmux send-keys -t "${SESSION}:0.1" '. ~/workspace/banpaku_scripts/jetson/docker_setup_commander.sh' C-m
sleep 1
echo "Pane 1: Starting tf2_echo for world->base_link ..."
tmux send-keys -t "${SESSION}:0.1" 'ros2 run tf2_ros tf2_echo world base_link' C-m
...
tmux send-keys -t "${SESSION}:0.2" '. ~/workspace/banpaku_scripts/jetson/docker_setup_commander.sh' C-m

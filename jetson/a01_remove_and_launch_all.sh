...
tmux send-keys -t "${SESSION}:0.0" '. ~/workspace/banpaku_scripts/jetson/start_sensors.sh' C-m
...
tmux send-keys -t "${SESSION}:0.2" '. ~/workspace/banpaku_scripts/jetson/set_exposure_and_remove_docker_container.sh' C-m
echo "Pane 2: Launching Docker..."
tmux send-keys -t "${SESSION}:0.2" '. ~/workspace/banpaku_scripts/jetson/run_docker.sh' C-m
sleep 1
...
tmux send-keys -t "${SESSION}:0.2" ". ~/workspace/banpaku_scripts/jetson/docker_run_local_discovery_server.sh" C-m

# Examine in Pane 4: run remote discovery server
echo "Pane 4: Launching Docker environment..."
tmux send-keys -t "${SESSION}:0.4" '. ~/workspace/banpaku_scripts/jetson/run_docker.sh' C-m
sleep 1
tmux send-keys -t "${SESSION}:0.4" ${USER_PASS} C-m
sleep 1
echo "Pane 4: Starting remote discovery server..."
tmux send-keys -t "${SESSION}:0.4" '. ~/workspace/banpaku_scripts/jetson/docker_run_remote_discovery_server.sh' C-m
...
tmux send-keys -t "${SESSION}:0.1" '. ~/workspace/banpaku_scripts/jetson/run_docker.sh' C-m
sleep 1
tmux send-keys -t "${SESSION}:0.1" ${USER_PASS} C-m
sleep 1
echo "Pane 1: Starting AP service..."
tmux send-keys -t "${SESSION}:0.1" '. ~/workspace/banpaku_scripts/jetson/docker_run_ap.sh' C-m

# Examine in Pane 3: tf2_echo
echo "Pane 3: Launching Docker environment..."
tmux send-keys -t "${SESSION}:0.3" '. ~/workspace/banpaku_scripts/jetson/run_docker.sh' C-m
sleep 1
tmux send-keys -t "${SESSION}:0.3" ${USER_PASS} C-m
sleep 1
echo "Pane 3: setup SUPER_CLIENT"
tmux send-keys -t "${SESSION}:0.3" '. ~/workspace/banpaku_scripts/jetson/docker_setup_commander.sh' C-m
sleep 1
echo "Pane 3: Starting tf2_echo for world->base_link ..."
tmux send-keys -t "${SESSION}:0.3" 'ros2 run tf2_ros tf2_echo world base_link' C-m

# Examine in Pane 5: commander
echo "Pane 5: Launching Docker environment..."
tmux send-keys -t "${SESSION}:0.5" '. ~/workspace/banpaku_scripts/jetson/run_docker.sh' C-m
sleep 1
tmux send-keys -t "${SESSION}:0.5" ${USER_PASS} C-m
sleep 1
echo "Pane 5: setup SUPER_CLIENT"
tmux send-keys -t "${SESSION}:0.5" '. ~/workspace/banpaku_scripts/jetson/docker_setup_commander.sh' C-m
